...
class PegasusRBM(nn.Module):
    def __init__(self, n_latent_nodes, **kwargs):
        ...
        for key in itertools.combinations(range(self._num_partitions), 2):
            ...
            self._weight_dict[str_key] = nn.Parameter(torch.randn(self._nodes_per_partition,
                                                                  self._nodes_per_partition), requires_grad=True)

        # Dict of RBM biases for each partition
        for i in range(self._num_partitions):
            self._bias_dict[str(i)] = nn.Parameter(torch.randn(self._nodes_per_partition), requires_grad=True)
