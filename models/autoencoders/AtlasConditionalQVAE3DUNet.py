...
from models.networks.EncoderCond import EncoderHierarchyPB_BinEv2, EncoderBlock3DUNETv1
from models.networks.DecoderCond import DecoderCNNPB, DecoderCNNPBv2, DecoderCNNPBv3, DecoderCNNPBv4, DecoderCNNPBv4_HEMOD, DecoderCNNPB_HEv1, DecoderCNNPB3Dv1, DecoderCNNPB3Dv2, DecoderCNNPB3Dv3, DecoderCNNPB3DUNETv1
...
class AtlasConditionalQVAE3DUNet(GumBoltAtlasPRBMCNN):
    """
    GumBolt
    """

    def __init__(self, **kwargs):
        super(AtlasConditionalQVAE3DUNet, self).__init__(**kwargs)
        self._model_type = "AtlasConditionalQVAE3DUNet"
    ...
    def _create_encoder(self):
        ...
        return EncoderHierarchyPB_BinEv2(encArch=self._config.model.encodertype,
            input_dimension=self._flat_input_size,
            n_latent_hierarchy_lvls=1, # for UNet testing
            n_latent_nodes=self.n_latent_nodes,
            skip_latent_layer=False,
            smoother="Gumbel",
            cfg=self._config)
        # return EncoderBlock3DUNETv1(
        #     num_input_nodes=self._flat_input_size,
        #     n_latent_nodes=self.n_latent_nodes
        # )
    ...
    def _create_decoder(self):
        ...
        if self._config.model.decodertype == "SmallPB":
            ...
            return DecoderCNNPB3Dv3(node_sequence=self._decoder_nodes,
                              ...
                              num_output_nodes = self._flat_input_size,
                              cfg=self._config)
        elif self._config.model.decodertype == "SmallPB3DUNetv1":
            return DecoderCNNPB3DUNETv1(node_sequence=self._decoder_nodes,
                              activation_fct=self._activation_fct,
                              num_output_nodes = self._flat_input_size,
                              cfg=self._config)
    ...
    def forward(self, xx, is_training, beta_smoothing_fct=5, act_fct_slope=0.02):
        ...
        out.beta, out.post_logits, out.post_samples, skip_connections = self.encoder(x, x0, is_training, beta_smoothing_fct)
        # out.post_samples, skip_connections = self.encoder(x, x0)
...
#         post_samples = torch.cat([post_samples, x[1]], dim=1)
        ...
        output_hits, output_activations = self.decoder(post_samples, x0, skip_connections)
...
#         #         "entropy":entropy, "pos_energy":pos_energy, "neg_energy":neg_energy}
    ...
    def kl_divergence(self, post_logits, post_samples, is_training=True):
        ...
        # clone.detach post_zetas to a new var
        # bp on new var
        ...
        # comment out this block the set neg_energy = pos_energy 
    ...
    def generate_samples(self, num_samples: int = 128, true_energy=None, measure_time=False):
        ...
        for _ in range(n_iter):
            ...
            tensor_sizes = [
                (p0_state.size(0), 32, 22, 8, 16),
                (p0_state.size(0), 64, 10, 7, 16),
                (p0_state.size(0), 128, 8, 5, 7),
                (p0_state.size(0), 512, 3, 3, 3),
                (p0_state.size(0), 1208, 1, 1, 1)
            ]

            random_tensors = [torch.randn(size, device = p0_state.device) for size in tensor_sizes]
            
            hits, activations = self.decoder(prior_samples, true_e, random_tensors)
            beta = torch.tensor(self._config.model.beta_smoothing_fct,
                                dtype=torch.float, device=hits.device)
            sample = self._inference_energy_activation_fct(activations) \
                * self._hit_smoothing_dist_mod(hits, beta, False)
