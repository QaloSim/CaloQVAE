...
class CRBMStats():
    """
    This module is for conditionalized PRBM stats generation
    """
    ...
    def block_gibbs_sampling_ais(self, beta, p0_state=None, p1_state=None, p2_state=None, p3_state=None):
        ...
        p0_bias = torch.zeros_like(p_bias['0']) # p_bias['0']
        p1_bias = p_bias['1']
        p2_bias = p_bias['2']
        p3_bias = p_bias['3']
...
#         p3_state_i.to(dev)
    ...
    def AIS(self, nbeta=20.0):
        ...
        self.lnZa = np.sum([torch.log(1 + torch.exp(-self._prbm.bias_dict[i] )).sum().item() for i in ['1','2','3']])
    ...
    def RAIS(self, nbeta=20.0):
        self.lnZb = np.sum([torch.log(1 + torch.exp(-self._prbm.bias_dict[i])).sum().item() for i in ['1','2','3']])
        FreeEnergy_ratios = 0.0
        Δbeta = 1/nbeta
...
def get_Zs_cond(run_path, engine, dev, step = 10):
    ...
    en_encoded_list = []
    offset = fn["min"]
    for i in range(1 + offset,fn["size"]+1 + offset,step):
...
#     filenames["size"] = int(len(files)/3)
    l = list(np.unique([files[i].split("_")[1] for i in range(len(files)) ]))
    filenames["min"] = min([int(item) for item in l])-1
    filenames["size"] = len(l)
...
#     for key in list(filenames.keys())[:-2]: #filenames.keys():
