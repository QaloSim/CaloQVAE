        ...
        with torch.set_grad_enabled(is_training):
            for batch_idx, (input_data, label) in enumerate(data_loader):
                ...
                if is_training:
                    ...
                    if "kl_loss" not in batch_loss_dict.keys():
                        batch_loss_dict["loss"] = batch_loss_dict["ae_loss"] + batch_loss_dict["hit_loss"]
                        batch_loss_dict["ahep_loss"] = batch_loss_dict["ae_loss"] + batch_loss_dict["hit_loss"]
...
#                         logger.info(input_images[0].shape)
                ...
                if mode == "train" and (batch_idx % valid_batch_idx) == 0:
                    ...
                    if "kl_loss" not in valid_loss_dict.keys():
                        valid_loss_dict["loss"] = valid_loss_dict["ae_loss"] + valid_loss_dict["hit_loss"]
                        valid_loss_dict["ahep_loss"] = valid_loss_dict["ae_loss"] + valid_loss_dict["hit_loss"]
