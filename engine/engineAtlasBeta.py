...
class EngineAtlasBeta(EngineCaloV3):

    def __init__(self, cfg, **kwargs):
        logger.info("Setting up engine Atlas.")
        super(EngineAtlasBeta, self).__init__(cfg, **kwargs)
        self.init_beta_QA()
    ...
    def fit(self, epoch, is_training=True, mode="train"):
        ...
        with torch.set_grad_enabled(is_training):
            for batch_idx, (input_data, label) in enumerate(data_loader):
                ...
                in_data_flat = [input_data]
                in_data = input_data.to(self._device)
                true_energy = label.to(self._device)
    ...
    def _energy_encoded_data(self):
        ...
        with torch.no_grad():
            for xx in data_loader:
                in_data_flat = [xx[0]]
                in_data = xx[0].to(self._device)
                true_energy = xx[1].to(self._device)
