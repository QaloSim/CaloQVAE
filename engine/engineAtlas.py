...
#             encoded_data_energy = self._energy_encoded_data()
            encoded_data_energy = self._energy_encoded_data_cond()
    ...
    def _energy_encoded_data_cond(self):
        partition_size=self._config.model.n_latent_nodes
        data_loader = self.data_mgr.val_loader
        energy_encoded_data = []
        ...
        with torch.no_grad():
            for xx in data_loader:
                ...
                beta, post_logits, post_samples = self.model.encoder(in_data, true_energy, False)
                post_samples = torch.cat(post_samples, 1)
                u = self.model.convert_inc_eng_to_binary(true_energy)
                post_samples_energy = self.model.stater.energy_samples(u, post_samples[:,0:partition_size], post_samples[:,partition_size:2*partition_size], 
                                                         post_samples[:,2*partition_size:3*partition_size], 1.0 )
                energy_encoded_data.append(post_samples_energy.detach().cpu())
